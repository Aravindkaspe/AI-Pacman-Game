...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for beliefs, alive in zip(self.ghostBeliefs, gameState.getLivingGhosts()[1:]):
            if alive:
                ghostPos = beliefs.argMax()
        ...
        bestAction = None
        minDistanceAction = float('inf')
        for action in legalActions:
            successorPos = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(successorPos, closestGhostPos)
            if distance < minDistanceAction:
                minDistanceAction = distance
                bestAction = action
                if minDistanceAction == 0:
                    break
